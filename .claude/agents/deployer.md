...
⚠ **不要自己算「第 N 版」然後當成答案講給 Andy。** 2026-09-23 實測對不上：
工作流是 `gh api .../runs?created>=<台北當日 00:00>` 取 `total_count`，
但用同一套算法推出來的數字跟 Andy 螢幕上看到的差很多（推 14、他看到 5）。
差在哪還沒查清楚，**所以這個數字不可靠，不准拿它當交付證據**。

**改用「建置時間」當對照鍵** —— 徽章上除了版次還印建置時間，那個是你算得準的：
就是 `deploy-pages` 那一步完成的台北時間。回報時講：
> 徽章上的**建置時間**應該是「台北 18:05」左右。版次那個數字我算不準，請以時間為準。

回報時一律這樣寫：
> 依部署紀錄，這次發布的是 commit `<sha>`，建置時間台北 `<HH:MM>`。
> **這個容器打不開線上網頁，所以我沒有親眼讀到徽章。**
> 請你重新整理看一眼徽章上的**建置時間**對不對得上；
> 如果還是舊的，那是瀏覽器快取（按 Ctrl+F5），不是沒部署到。
