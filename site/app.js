    ...
    chart('breadth', {
      ...
      series: [
        { type: 'gauge', startAngle: 200, endAngle: -20, min: 0, max: 100, radius: 80, center: ['50%', 118],
          ...
          detail: { valueAnimation: true, offsetCenter: [0, -4], fontSize: 26, fontFamily: NUM_FONT,
  ...
  function openRotZoom(rrg, back0) {
    ...
    openZoom('足跡輪盤', (body, chipBox, close) => {
      ...
      const draw = () => renderRotClock(rows0, ROT_SPAN, 'zoomBody', false,
        { pick: rotPickSet(), frame: rotFrame, span: Math.max(0, ROT.days - rotFrame), trail: ROT.trail, tmode: 'focus' });
      rotZoomDraw = draw;
      draw();
