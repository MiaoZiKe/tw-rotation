  ...
  function termsDoc() {
    ...
    const secs = [
      ...
      { h: '智慧財產權與授權', b: ol([
        ...
        fill('本服務之程式碼公開於{repo_url}，其使用條件依該處之授權文件（LICENSE）辦理。'),
...
@media (prefers-reduced-motion:reduce){
  ...
  function buildFooter() {
    ...
    f.innerHTML = '<p class="sf-dis"><b>免責聲明</b>　本站為公開資料之整理、計算與視覺化工具，不是證券投資顧問事業，'
      ...
      + '<a href="' + esc(CFG.repo_url || '') + '" target="_blank" rel="noopener">原始碼與演算法</a></nav>';
    main.appendChild(f);
  ...
  const STEPS = [
    ...
    { t: '③ 何時進場', d: '看技術面條件：是回檔承接還是突破追價。這一步列的是「符合條件」，不是推薦。', sel: '#ovCandCard' },
