...
(function () {
  ...
  const STG = [
    ...
    { id: 'sw_polish', t: ['⑤ 拋光', '鏡面，一條直線'], u: ['吃研磨液、拋光', '墊、鑽石碟'], amp: 0, dmg: false },
  ];
  ...
  function areaC() {
    const g = STG.map((o, i) => {
      ...
      return part(o.id, body)
        + o.t.map((s, j) => T(x, SG.top - 30 + j * 16, s, 'lbl')).join('')
    ...
    return `<g>
      ${T(16, 740, '切完的片不能用 —— 表面一級一級磨到鏡面', 'hd')}
      ${T(16, 758, '粗糙度只會變細，順序不能顛倒；磊晶一定在拋光之後。', 'sub')}
      ${g}
  ...
  function areaF() {
    ...
    return `<g>
      ...
      ${pbar()}
      ${part('sw_reclaim', R(360, 1116, 284, 70, 'var(--dg-frame-f)', 'part', 7)
      + T(372, 1138, '（灰）再生晶圓', 'lbl', null, `fill:${C.mute}`)
      + T(372, 1158, '用過的測試片重新磨回可用。', 'sub', null, `fill:${C.mute}`)
      + T(372, 1176, '不是主線的一段。', 'sub', null, `fill:${C.mute}`))}
      ${LN(`M360,1151 L${back},1151 L${back},${FB.y + FB.row + FB.h + 6}`, C.mute, 1.4, ' stroke-dasharray="5 4"')}
      ${PA(`M${back},${FB.y + FB.row + FB.h} L${back - 5},${FB.y + FB.row + FB.h + 10} L${back + 5},${FB.y + FB.row + FB.h + 10}Z`, C.mute)}
      ${T(back + 12, 1146, '← 另一條路：回收再利用', 'cap', null, `fill:${C.mute}`)}

      ${frame(16, 1210, 628, 168)}
      ${T(30, 1234, '這張圖上的段，台股是誰（信心：中，來源為媒體與公司網站整理）', 'hd')}
      ${T(30, 1256, '① 多晶矽原料：本圖查不到台股的具名對應 —— 查不到就寫查不到。', 'sub')}
      ${T(30, 1276, '② CZ 長晶／③ 外圓磨＋notch／④ 線鋸切片／⑤ 倒角研磨蝕刻／⑥ 拋光清洗：', 'sub')}
      ${T(30, 1294, '　 6488 環球晶、3532 台勝科、6182 合晶。③～⑤ 是矽晶圓廠的內部工序，', 'sub')}
      ${T(30, 1312, '　 本圖查不到專做這幾道工序的台股獨立廠商。', 'sub')}
      ${T(30, 1332, '⑦ 磊晶（選配）：6488 環球晶（產品含磊晶片）、6182 合晶（產品含磊晶矽晶圓）。', 'sub')}
      ${T(30, 1352, '（灰）再生晶圓：8028 昇陽半導體（以再生晶圓起家）、1560 中砂（再生晶圓是其三大事業', 'sub')}
      ${T(30, 1370, '　 之一，掛在「前段製程材料」族群）。⑥ 拋光吃的耗材不是這一格的公司做的。', 'sub')}</g>`;
  }
  ...
  function siliconWafer() {
    ...
    return `<svg class="dg dgm rs dgsw2" viewBox="0 0 ${CW} 1400" width="100%" style="display:block">${D.STYLE}
      <style>
