...
(function () {
  ...
  function furnace() {
    ...
    // ★★ 提拉方向：朝上（C2）
    g.push(part('sw_pull', upArrow(F.rodX + 74, 214, 92) + T(F.rodX + 84, 176, '一邊轉、一邊往上拉', 'lbl')));
  ...
  function areaA() {
    return `<g>${frame(16, 68, 452, 434)}
      ${T(32, 92, '一、CZ 提拉法長晶爐（剖面）', 'hd')}
      ${T(32, 110, '熔湯在下、籽晶在上，一邊轉一邊往上拉 —— 這就是「提拉法」。', 'sub')}
      ${furnace()}
      ${T(32, 468, '★ 看得到熔湯液面，才是 CZ 提拉爐。碳化矽／氮化鎵用的是昇華法', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(32, 486, '　 （PVT，沒有液面），爐子長得不一樣 —— 見「第三代半導體」那張。', 'sub', null, 'fill:var(--dg-warn)')}</g>`;
  }
  ...
  const STG = [
    { id: 'sw_ascut', t: '① 切片後', s: '鋸痕 ＋ 加工損傷層', u: '這樣的片不能用', amp: 9.0, dmg: true },
    { id: 'sw_edge', t: '② 倒角', s: '只把邊緣磨圓', u: '表面幾乎沒動', amp: 7.2, dmg: true },
    { id: 'sw_lap', t: '③ 研磨（lapping）', s: '鋸齒變成小很多的波浪', u: '吃研磨液與磨粒', amp: 4.6, dmg: true },
    { id: 'sw_etch', t: '④ 蝕刻', s: '起伏更小，損傷層被移除', u: '吃蝕刻化學品', amp: 2.2, dmg: false },
    { id: 'sw_polish', t: '⑤ 拋光（CMP）', s: '做到鏡面，是一條直線', u: '吃研磨液、拋光墊、鑽石碟', amp: 0, dmg: false },
  ];
  const SG = { w: 146, gap: 8, x0: 22, top: 600, bot: 690 };
    ...
    const g = STG.map((o, i) => {
      ...
      const body = R(x, SG.top, SG.w, SG.bot - SG.top, 'var(--dg-si-2)', 'part', 2)
        ...
        + (o.id === 'sw_edge'
          ...
          : '');
      return part(o.id, body) + T(x, SG.top - 40, o.t, 'lbl') + T(x, SG.top - 22, o.s, 'sub')
        + T(x, SG.bot + 20, o.u, 'cap');
    }).join('');
    // ⑥ 磊晶（選配的最後一段）：在拋光面上長一層新的矽，**比基板薄很多**、中間有一條清楚的界線
    const ex = SG.x0 + 5 * (SG.w + SG.gap);
    const epi = part('sw_epi',
      R(ex, SG.top + 6, SG.w, (SG.bot - SG.top) - 6, 'var(--dg-si-2)', 'part epibase', 2)
      + R(ex, SG.top, SG.w, 6, 'var(--dg-si)', 'part epi', 1)
      + LN(`M${ex},${SG.top + 6} L${ex + SG.w},${SG.top + 6}`, 'var(--dg-cover)', 1.6));
    return `<g>${frame(16, 520, 940, 230)}
      ${T(32, 544, '三、切完的片不能用 —— 表面一級一級磨到鏡面（粗糙度只會變細，順序不能顛倒）', 'hd')}
      ${g}
      ${epi}${T(ex, SG.top - 40, '⑥ 磊晶（選配）', 'lbl')}${T(ex, SG.top - 22, '在鏡面上長一層新的矽', 'sub')}
  ...
  const SAY = [
    ['① 晶圓不是「切出來」的，是「拉出來」的。',
      ['多晶矽在石英坩堝裡熔成一鍋湯，一顆籽晶沾上去、一邊轉一邊往上拉，',
        '熔湯就照著籽晶的晶格重新排列、凝固成一根單晶圓柱。']],
    ['② 拉出來的柱子比目標尺寸大一圈，要先磨小再切。',
      ['先拉到約 306 mm、再外圓磨到 300 mm，然後刻 notch、用多線鋸切片。',
        '（只有一個來源，實際餘量各廠不同，畫面上標「約」。）']],
    ['③ 切完的片不能用，要一級一級磨到鏡面。',
      ['切片 → 倒角 → 研磨 → 蝕刻 → 拋光 → 清洗，粗糙度一路變細、順序不能顛倒；',
        '要長磊晶的話，磊晶一定在拋光之後 —— 粗糙面上長不出可用的單晶層。']],
    ['④ 12 吋的面積是 8 吋的 2.25 倍，而邊緣浪費掉的比例更小。',
      ['這兩件事加起來，就是「晶圓越大越划算」。']],
  ];
  function areaE() {
    let y = 812;
    const rows = SAY.map((o) => {
      const s = T(496, y, o[0], 'lbl') + o[1].map((t, i) => T(496, y + 18 + i * 18, t, 'sub')).join('');
      y += 22 + o[1].length * 18 + 12;
      return s;
    }).join('');
    return `<g>${frame(480, 766, 476, 314)}
      ${T(496, 790, '五、看完要能自己講出這四句', 'hd')}
      ${rows}</g>`;
  }
  ...
  function siliconWafer() {
    return `<svg class="dg dgm dgsw" viewBox="0 0 ${W} 1640" width="100%" style="display:block">${D.STYLE}
      ${T(16, 26, '矽晶圓：從熔湯到一片鏡面', 'ttl')}
      ${T(16, 46, '一鍋熔湯 → 一根拉出來的單晶柱子 → 一片磨到鏡面的圓片。這張圖止於「一片可以進晶圓廠的晶圓」。', 'cap')}
      ...
      ${T(16, 1556, '示意圖，非實物比例｜爐體與各層厚度均為誇張放大', 'cap')}
      ${T(16, 1574, '本圖講矽晶圓；碳化矽與氮化鎵用的是昇華法（PVT），爐子長得不一樣，見「第三代半導體」那張', 'cap')}
      ${T(16, 1592, '本圖止於「一片可以進晶圓廠的晶圓」；之後的製程見「晶圓代工」那張', 'cap')}
      ${T(16, 1610, '本圖只講半導體級矽晶圓，不含太陽能矽晶圓；不寫任何良率、市占率、產能、單價與漲價幅度', 'cap')}
    </svg>`;
  }
