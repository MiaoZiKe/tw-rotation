...
(function () {
  ...
  function pcbStackup() {
    ...
    return `<svg class="dg dgm dgpcb" viewBox="0 0 980 1446" width="100%" style="display:block">${DG.STYLE}${CSS}
      ...
      ${DG.fold('pcb2', '② 四種孔各解決一件事｜③ 走線：線寬線距、差動成雙、蛇行等長', '通孔／背鑽／盲孔／埋孔四種跨距並排、HDI 階數怎麼數、四種走線的頂視小圖', `
      ...
      ${DG.fold('pcb4', '④ 材料為什麼會變成瓶頸：銅箔稜面與玻纖織紋', '介電（Df）與導體（銅箔稜面）兩邊一起上來的損耗、玻纖織紋效應', `
      <!-- 材料：銅箔稜面、玻纖織紋、以及「這不是 PCB」的對照 -->
      <text class="hd" x="16" y="878">材料為什麼會變成瓶頸：損耗從介電（Df）與導體（銅箔稜面）兩邊一起上來</text>
      ${matRow()}
      `)}

      ${DG.fold('pcb5', '⑤ 製程五格｜層數為什麼一直往上加｜誰做哪一塊', '五格製程流程列（CCL 廠與 PCB 廠的分界線）、兩塊結論框、三格誰做哪一塊與免責', `
  ...
  window.DG.register('pcb_rigid', {
    ...
    parts: {
      ...
      pcb_foil_top: {
        name: '頂層銅箔上的線路',
        desc: '頂層銅箔上蝕出來的線路（頂視）。外層線要細，用的銅箔就要更薄，不然蝕刻蝕不出側壁。',
        cos: ['8358', '4989', '1303'],
        items: ['HVLP 銅箔'],
      },
      foil_rough: {
      ...
      pcb_core: { name: '芯板 core（銅－介電－銅）', desc: '芯板：已固化、兩面覆銅的板材。CCL 廠賣的就是它 —— 交到 PCB 廠手上時線路還沒蝕。' },
      ...
      pcb_mask: { name: '防焊（solder mask）', desc: '防焊：蓋住不該吃錫的銅，焊墊處開窗。順序上一定在表面處理之前，反過來金會鍍到整片銅上。' },
      pcb_enig: { name: '表面處理（ENIG＝化鎳浸金）', desc: '表面處理（ENIG＝化鎳浸金）：露出來的銅上面依序是銅 → 鎳 → 金，鎳負責擋擴散。同一塊板子選 ENIG／OSP／ENEPIG 是成本與保存期的取捨。' },
      pcb_pth: { name: '① 機械通孔 PTH ＋ 殘端', desc: '機械通孔（PTH）：上下都貫穿，最便宜。但訊號只走到中間某一層，剩下那一截孔銅就是一根沒接東西的天線。' },
      pcb_backdrill: { name: '② 背鑽（back drill）', desc: '背鑽：用比原孔大一號的鑽頭，從背面把 PTH 的殘端鑽掉。工程上鑽不到零，一定會留一小段。' },
      pcb_blind: { name: '③ 雷射盲孔（micro via）', desc: '雷射盲孔（micro via）：上寬下窄的碗狀、只穿一層介電、不貫穿。外層才佈得下更密的線。' },
