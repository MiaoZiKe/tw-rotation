...
(function () {
  ...
  function subs() {
    ...
    g.push(part('sa_scale', LN(`M${mx(3) + MW},${PB - 18 - 12 * UNIT} H624 V252`, C.warn, 1.4, ' stroke-dasharray="5 5"')
      + T(608, PB - 18 - 12 * UNIT - 8, '同一個刻度', 'sub', 'end', `fill:${C.warn}`)));   // ★ 2026-09-26：原本右端在 624，正好被 5 號編號圓點蓋住
    return g.join('');
  }
  ...
  /* ★ 2026-09-26 覆蓋普查（scripts/_dg_overlap.py）：合約負債右邊那四行原本照舊畫布一整句一行，
     在 x 306 起筆、伸出畫布最多 179px → 交給 D.para 照最壞字寬斷行，框高照內容長（至少 150，左邊那塊合約負債照原樣）。*/
  const DEF_T = ['客戶年繳，錢一次進帳 —— 但依會計準則不能一次認列營收，要按月把服務「交付」出去才逐月認列。沒認列的那一段掛在合約負債（遞延收入）。',
    '★ 所以合約負債變大通常是好事：它是「已經收到錢、還沒認列的未來營收」。',
    '★ 這是 IFRS 15 的通則，不是任何一家公司的說法；本圖不引用任何一家的實際金額。'].map((t) => ({ t, style: /^★/.test(t) ? `fill:${C.warn}` : '' }));
  const DEF_H = Math.max(150, 34 + D.para(0, 0, DEF_T, 644 - 306 - 14).h + 6);
  ...
  function areaMargin() {
    ...
    return T(16, y0 - 12, '毛利結構：一樣是一塊錢營收，成本長得不一樣', 'hd')
      ...
      + part('sa_defer', frame(16, y0 + 216, 628, DEF_H)
        ...
        + D.para(306, y0 + 250, DEF_T, 644 - 306 - 14).svg);
  }
  ...
  function saas() {
    return `<svg class="dg dgm rs dgsa" viewBox="0 0 ${CW} 1480" width="100%" style="display:block">${D.STYLE}
      ...
      ${project()}
      ${T(16, 247, '② 訂閱制：每個月一小格，疊成一道階梯', 'hd')}   <!-- ★ 2026-09-26：原本 244，閱讀模式字頂壓到上一格的陰影 -->
      ${subs()}
      ${conclusion()}
