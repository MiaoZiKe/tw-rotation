...
(function () {
  ...
  function areaVariants() {
    return `<g>
      ${T(16, 380, '① 另外兩種做法：溝槽閘（消掉 JFET 區）、GaN-on-SiC（緩衝層薄）、Cascode（常關）', 'hd')}
      ${T(24, 404, '溝槽閘：把閘極挖進去', 'lbl')}
      ${T(24, 422, '兩格同一比例、同一畫法，只有閘極那一段不同。', 'sub')}
      <g transform="translate(0,310)">${sicCell(24, true)}</g>
      ${T(300, 404, 'GaN-on-SiC：緩衝層薄', 'lbl')}
      ${T(300, 422, '長在 Si 上晶格差約 17%、要厚過渡；', 'sub')}
      ${T(300, 440, '長在 SiC 上只差約 3.5%，兩層就夠。', 'sub')}
      ${ganCell(300, 160, 506, 2, C.si2, 'wbg_gan_sic', false)}
      ${T(300, 630, '（這一格只畫疊構，電極與 p-GaN 見主畫面那一格）', 'cap')}
      ${T(500, 404, 'Cascode：控制端是那顆 Si', 'lbl')}
      ${T(500, 422, '低壓 Si 串一顆常開 GaN；', 'sub')}
      ${T(500, 440, '對外控制的是那顆 Si 的閘極。', 'sub')}
      ${cascode(524, 486)}
      ${T(16, 660, '示意圖，非實物比例｜溝槽深度、緩衝層層數與 Cascode 的封裝內配置均為示意。', 'cap')}</g>`;
  }
  function areaMarket() {
  ...
  function areaFlow() {
    return `<g>
      ...
      ${T(30, 1658, '另有 5347 世界先進取得台積電 GaN 技術授權（來源：媒體報導，2025–2026），但它掛在別的族群。', 'sub')}</g>`;
  }
  ...
  function wideBandgap() {
    return `<svg class="dg dgm rs dgwbg" viewBox="0 0 ${CW} 1700" width="100%" style="display:block">${D.STYLE}
      ...
      ${T(30, 330, '平面閘（DMOS）：留著 JFET 區；溝槽閘見第 ① 段。', 'cap')}
      ${T(30, 348, '青色箭頭＝電流，垂直貫穿整片晶片（正面 → 背面）。', 'cap')}
      ${T(306, 330, 'GaN-on-Si：晶格差得多，緩衝層要厚（畫 6 層）。', 'cap')}
      ${T(306, 348, '青色箭頭＝電流，橫著在 2DEG 裡跑 —— 與左邊差 90 度。', 'cap')}
      ...
      ${note({ side: 'l', order: 99, title: '這張圖不畫的事', lines: ['射頻 GaN 與 LED（同樣是 GaN，但應用、客戶與台股完全不同一批）；穩懋與宏捷科（一次搜尋摘要把這兩檔直接譯成兩家美國公司，沒有獨立查證過，所以不畫）；任何良率、成本、市占率與產能數字。'] })}
