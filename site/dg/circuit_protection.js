...
(function () {
  ...
  function circuitProtection() {
    ...
    return `<svg class="dg dgm rs dgcp dg1" viewBox="0 0 ${CW} ${H}" width="100%" style="display:block">${D.STYLE}
      ...
      ${cells()}
      <!-- ★ 2026-09-26 覆蓋普查（scripts/_dg_overlap.py）：四格底下的說明原本每行 11 個字寫死，最右那格（x 506 起、只剩 128 寬）
           閱讀模式伸出框 17px、伸出畫布。改成每格照自己的寬度交給 D.para 斷行（最多五行，框底在 582，放得下）；字一個都沒刪。 -->
      ${D.para(26, 494, '鎳電極箔 ×2 夾住高分子基體，外面包一層絕緣；黑點是導電碳黑。', 144, { lh: 17 }).svg}
      ${D.para(190, 494, '金屬氧化物燒結的陶瓷本體＋兩個相對面電極＋兩根引線。沒有 PN 接面。', 140, { lh: 17 }).svg}
      ${D.para(348, 494, '一堆大小不一的 ZnO 晶粒；擋電流的是晶粒之間的界面（晶界）。', 140, { lh: 17 }).svg}
      ${D.para(506, 494, '半導體 PN 接面：P 區／空乏區／N 區＋上下金屬電極。超過門檻就崩潰。', 126, { lh: 17 }).svg}
      ...
      <!-- ★ 2026-09-26 覆蓋普查：7 號原本在晶片左緣正中（y 98），卡片被排到右欄時引線沿 y 98 橫著劃過「IC」兩個字 → 移到晶片左下角 -->
      ${card({ side: 'l', no: 7, part: 'cp_ic', color: CC.line, ax: 598, ay: LY + 20, title: 'IC（被保護的對象）', sub: '不是任何真實晶片的外觀；整條路徑就是為了它而存在' })}
