...
(function () {
  ...
  /* 層厚 26→20、層間 8→7、整疊起點 150→92（Andy 2026-09-22：「圖片及文字縮小一半…
     希望能一次看到完整資訊」）。**只縮幾何、不縮字**；
     H5 要求的「base die 比 core die 厚」仍然成立（28 vs 20 ＝ 1.4 倍）。*/
  const H_CORE = 20, H_BASE = Math.round(H_CORE * 1.4), H_GAP = 7;
  /* 疊構放在框的左半邊（40～268），右半邊 296～460 留給短標籤 ——
     標籤伸出框外是字級與重疊都量不到的一種壞法，所以寬度先算好再寫字。*/
  const SX = 40, SW = 228, TOP = 92;
  ...
  function hbmStack() {
    ...
    g.push(`<g pointer-events="none">
      ${tag(110, G.cores[0] + 10, 'core die（DRAM）')}
      ${tag(158, G.cores[2] + 10, 'TSV 穿矽孔')}
      ${tag(206, G.gaps[3] + 3, '微凸塊（每兩層）')}
      ${tag(254, G.base + H_BASE / 2, 'base die（邏輯）')}
      ${tag(298, G.bottom + 7, '對外凸塊')}</g>`);
  ...
  function areaA() {
    const G = stackGeom();
    /* 說明行一行最多 34 個字（框內寬 418px）—— 超過就會伸進右邊那一框。
       這一區的框是 52～372，四行排在 306～354，全部在框內。*/
    return `<g>${frame(16, 52, 446, 320)}
      ${T(32, 74, '一、HBM 那一疊裡面是什麼（主角）', 'hd')}
      ${hbmStack()}
      ${T(32, 306, '本圖畫 ' + N_CORE + ' 層 core die 示意（實際依世代而定）；', 'sub')}
      ${T(32, 322, (N_CORE + 1) + ' 層晶粒就有 ' + N_CORE + ' 排微凸塊，且與 TSV 上下對齊。', 'sub')}
      ${T(32, 338, '★ base die 比較淡，是因為它掛的是「晶圓代工」環節、', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(32, 354, '　 不是「HBM」—— 最底下那顆是代工廠做的。', 'sub', null, 'fill:var(--dg-warn)')}</g>`;
  }

  /* ================================================================ 區 B：兩種層間接合放大格
     §3-B B1 微凸塊那格**有一排凸塊**、混合鍵合那格**沒有凸塊**（兩格都畫凸塊＝這一區沒有存在意義）；
     B2 混合鍵合的兩層之間只有一條接合界線；B3 兩格的 die 同色同厚。*/
  const BD = { x: 60, w: 200, h: 28 };        // 兩格共用同一個 die 尺寸（B3）
  ...
  /* 這一區收在第 ③ 個章節裡，所以它的自然位置就直接寫在章節體的範圍上（558～756），
     不再另外套 translate —— 少一層換算就少一個對不齊的機會。
     兩格直的改橫的：剖面在左（60～260）、說明在中（290）、低信心的警語在右（620）。*/
  function areaB() {
    return `<g>${frame(16, 560, 940, 194)}
      ${T(32, 582, '三、兩層之間是怎麼接起來的 —— 差別就是「有沒有那一排凸塊」', 'hd')}
      ${part('hb_ubump_zoom', bondPanel(606, true), 'hbm')}
      ${T(290, 620, '① 微凸塊（microbump）', 'lbl')}
      ${T(290, 640, '兩層之間有一排凸塊，凸塊周圍填著填充材。', 'sub')}
      ${T(290, 658, '這是目前多數世代的作法。', 'sub')}
      ${part('hb_hybrid', bondPanel(690, false), 'hbm')}
      ${T(290, 704, '② 混合鍵合（hybrid bonding）', 'lbl')}
      ${T(290, 724, '兩層之間沒有凸塊 —— 銅墊直接對銅熔接，', 'sub')}
      ${T(290, 742, '只剩一條接合界線；間距更小、整疊也更薄。', 'sub')}
      ${T(620, 620, '★ 混合鍵合是各家在 HBM4 世代的路線之一，', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(620, 638, '　 各家做法不同（來源：產業媒體，2026）——', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(620, 656, '　 本圖不列「某家用 A、某家用 B」的對照表。', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(620, 682, '兩格的晶粒刻意同色同厚：只有接法不同。', 'cap')}
      ${T(620, 700, '本圖不畫 DRAM 單元結構（電容／字元線／位元線）。', 'cap')}</g>`;
  }
  ...
  /* 系統剖面搬到主畫面的右半邊（x 472～956），整組尺寸收一半。
     它的任務只有一句話：「HBM 是站在 GPU 旁邊，不是疊在 GPU 上面」——
     §0-B 明講這一格要刻意畫小、畫簡單，任何多加的細節都會跟既有的 CoWoS 圖打架。*/
  const SYS = { subT: 248, subB: 270, itT: 220, itB: 246, dieT: 160, dieB: 220 };
  /* 俯視小格放左邊（486～602）、系統剖面放右邊（616～946）。
     ⚠ 原本在剖面左側逐層標「HBM 堆疊／中介層／載板」三個標籤，
       實測跟俯視小格的說明撞在一起（12px）—— 而且那三個字上面那行已經寫過一次
       （「由下到上：載板 → 中介層 → 晶粒」）。照 Andy 的第 1 條「留一次」，
       三個標籤刪掉，改成剖面底下一句話。*/
  function system() {
    const S = SYS, g = [];
    g.push(part('hb_sub', R(616, S.subT, 304, S.subB - S.subT, 'var(--dg-pcb)', 'part', 3), 'adv_pkg'));
    // 中介層：裡面那些細密繞線就是它存在的理由（Y4）
    const wires = [];
    for (let i = 0; i < 9; i++) {
      const y = S.itT + 4 + i * 2.2;
      wires.push(`<path class="route" d="M702,${y} L734,${y}" stroke="var(--dg-cu)" stroke-width="1" fill="none"/>`);
      wires.push(`<path class="route" d="M800,${y} L834,${y}" stroke="var(--dg-cu)" stroke-width="1" fill="none"/>`);
    }
    g.push(part('hb_interposer', R(624, S.itT, 288, S.itB - S.itT, 'var(--dg-si-2)', 'part', 2)
      + wires.join(''), 'adv_pkg'));
    // ★★ HBM 與 GPU **並排**站在中介層上（Y2）：同一個底面 dieB，x 互不重疊
    const hbmBlk = (x) => R(x, S.dieT, 62, S.dieB - S.dieT, 'var(--dg-si)', 'part hblk', 2)
      + [0, 1, 2, 3, 4].map((i) => LN(`M${x + 4},${S.dieT + 6 + i * 9} L${x + 58},${S.dieT + 6 + i * 9}`, 'var(--dg-void)', 1.2)).join('')
      + R(x, S.dieB - 8, 62, 8, 'var(--dg-organic)', 'part hbase', 2);
    g.push(part('hb_hbm_pkg', hbmBlk(638) + hbmBlk(836), 'hbm'));
    g.push(part('hb_gpu', R(736, S.dieT, 62, S.dieB - S.dieT, 'var(--dg-die)', 'part gblk', 2)
      + T(767, S.dieT + 34, 'GPU', 'lbl', 'middle')));
    // 俯視小格（Y3）：GPU 在中間、HBM 在兩側，數量對稱
    const tv = 486;
    g.push(part('hb_topview', R(tv, 152, 116, 106, 'var(--dg-frame-f)', '', 6)
      + R(tv + 46, 170, 24, 70, 'var(--dg-die)', 'part tvg', 2)
      + [0, 1].map((i) => R(tv + 10, 172 + i * 36, 28, 28, 'var(--dg-si)', 'part tvh', 2)).join('')
      + [0, 1].map((i) => R(tv + 78, 172 + i * 36, 28, 28, 'var(--dg-si)', 'part tvh', 2)).join('')
      + T(tv + 58, 274, '俯視：GPU 在中間', 'cap', 'middle')));
    return g.join('');
  }
  function areaC() {
    return `<g>${frame(470, 52, 486, 320)}
      ${T(486, 74, '二、HBM 被放在哪裡（定位用，刻意畫得簡單）', 'hd')}
      ${T(486, 94, '★ HBM 是站在 GPU 旁邊，不是疊在 GPU 上面。', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(486, 114, '由下到上：載板 → 中介層 → 晶粒；中介層裡那些細密', 'sub')}
      ${T(486, 130, '繞線就是它存在的理由。', 'sub')}
      ${system()}
      ${T(616, 292, '左右兩塊是 HBM、中間是運算晶粒 —— 三者一起站在中介層上。', 'cap')}
      ${T(486, 326, '本格不畫補強環、模封與載板的內部層數 —— 那是「CoWoS 2.5D 封裝剖面」', 'cap')}
      ${T(486, 342, '與「先進封裝」那兩張的範圍。', 'cap')}</g>`;
  }
  ...
  function hbmStackFig() {
    /* 版面（Andy 2026-09-22：「圖片及文字縮小一半…希望能一次看到完整資訊」）
       ------------------------------------------------------------------
       主畫面留這張圖的兩個命題：**堆疊剖面**（最底下那顆不是記憶體）＋
       **系統剖面**（HBM 是站在 GPU 旁邊，不是疊上去）。
       兩種接合方式、台股標示帶收進章節列，預設收合、按了打得開。
       收合後高度 ＝ 496 ＋ 兩條列 × 46 ＋ 16 ＝ **604px**。

       ★ 「台股標示帶」是這張圖最重要的一段，收起來會不會把它藏掉？
         不會：**結論留在主畫面**（台股沒有 HBM 顆粒廠、唯一具名的是 base die 找台積電代工），
         逐段的章與名單收進章節。五行誠實性標示一行都沒有收起來。*/
    const S1 = 512, S2 = 768;
    return `<svg class="dg dgm dghb" viewBox="0 0 ${W} 1200" width="100%" style="display:block">${D.STYLE}
      ${T(16, 24, 'HBM：堆疊起來的記憶體與底下那顆邏輯晶粒', 'ttl')}
      ${T(16, 42, '把多顆 DRAM 疊起來、用一根根穿透的孔連成一整塊；最底下那顆是找晶圓代工廠做的邏輯晶粒。底下兩段預設收起來。', 'cap')}

      ${areaA()}${areaC()}

      ${T(16, 392, '★ 誰做的：台股沒有 HBM 顆粒廠（實際上是 SK hynix／Micron／Samsung）；唯一具名的位置是 base die —— SK hynix 的', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(16, 410, '　 HBM4 base die 採 2330 台積電 12 奈米邏輯製程（來源：產業媒體，2026）。逐段的章與名單見第 ④ 段。', 'sub', null, 'fill:var(--dg-warn)')}
      ${T(16, 434, '示意圖，非實物比例｜層厚、孔徑與凸塊尺寸均為誇張放大', 'cap')}
      ${T(16, 450, '堆疊層數為示意，實際層數依世代而定', 'cap')}
      ${T(16, 466, '本圖講「HBM 那一疊裡面是什麼」；HBM 被放進封裝的樣子見「半導體：CoWoS 2.5D 封裝剖面」那張', 'cap')}
      ${T(16, 482, '台股沒有 HBM 顆粒廠 —— 這不是漏畫，是這張圖要講的事', 'cap')}
      ${T(16, 498, '供應鏈資料的「HBM」環節底下只有外商，所以點零件列出來的是外商，這是刻意的', 'cap')}

      <!-- ================= ③ 兩層之間是怎麼接起來的（預設收合） ================= -->
      ${D.foldBar('hb2', S1, '③ 兩層之間是怎麼接起來的：微凸塊 vs 混合鍵合',
      '差別就是「有沒有那一排凸塊」；混合鍵合是銅對銅直接接，只剩一條界線')}
      <g class="dgbody" data-fold="hb2" data-y0="${S1 + 46}" data-y1="${S1 + 46 + 198}">${areaB()}</g>

      <!-- ================= ④ 台股在這條鏈上到底站在哪裡（預設收合） ================= -->
      ${D.foldBar('hb3', S2, '④ 台股在這條鏈上到底站在哪裡',
      '五段各是誰（兩段是紅章），以及南亞科與力成為什麼不畫在上面')}
      <g class="dgbody" data-fold="hb3" data-y0="${S2 + 46}" data-y1="${S2 + 46 + 369}">
        <g transform="translate(0,${S2 + 46 - 883})">
          ${areaD()}
