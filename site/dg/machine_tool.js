...
(function () {
  ...
  const PARTS = {
    mt_bed: { name: '床身與立柱（鑄件）', desc: '整台機器的地基：主軸切下去的力、工作台移動的慣性，最後都由它承受。★ 床身與立柱通常是一體的鑄件，所以底部是一個連續的 L 形；畫成兩塊各自站著的方塊就失去了「剛性是一體的」這件事。鑄件重、運費高、又要時效，所以在地供應本來就比較划算 —— 這是台中工具機聚落的底子。',
