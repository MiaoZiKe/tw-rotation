...
(function () {
  ...
  const DC_ITEMS = ['① 中壓配電盤（22.8 kV）：一排等高金屬櫃，每櫃一路饋線；第三櫃把抽出式斷路器拉出來一半，看得到導軌與一次接觸子。每櫃正面分三段：儀表／電驛室、斷路器室、電纜室。★ 同樣不降壓。',
    '② 乾式（模鑄）變壓器：降到 380／220 V —— 它沒有油箱、沒有散熱片、沒有儲油櫃，那三個「沒有」就是它的長相；罩子裡看得見三個直立樹脂線圈。',
    '③ UPS（撐到發電機起來那幾分鐘）',
    '④ 電池櫃：一層一層的電池模組抽屜，看得出分層與極柱',
    '⑤ PDU　⑥ 匯流排槽（懸吊在機櫃上方，插接箱往下拉線）',
    '⑦ 機櫃　⑧ 電源架（1U 裡並排可熱插拔模組）',
    '⑨ 直流匯流排。★ 這一站不列代號（屬「伺服器電源與 BBU」題材）。'];
  const DC_P = D.para(30, 244, DC_ITEMS, 600, { indent: '　 ' });
  function areaRoom() {
    return `<g>
      ...
      <!-- ★ 2026-09-26 覆蓋普查（scripts/_dg_overlap.py）：這一框的條列原本照舊畫布寫死斷行，閱讀模式伸出框 72～75px、伸出畫布 60px。
           改成每一條交給 D.para 照最壞字寬斷行（續行縮排對齊），框高照內容長；條列內容與順序一字未改。 -->
      <rect class="frame" x="16" y="198" width="628" height="${46 + DC_P.h}" rx="8"/>
