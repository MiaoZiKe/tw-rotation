...
(function () {
  ...
  const CELL_W = 304, CA_Y = 68, CA_H = 436;
  ...
  const TX = [
    ...
    {
      k: 'fin', x: 334, no: '二', nm: 'FinFET（鰭式）', fa: '閘極管到 3 面', wi: [5, 6],
      cap: ['把通道立起來變成一片鰭，閘極',
        '就能罩住頂面與兩個側面。',
        '鰭高畫成鰭寬的 4.75 倍 —— 又矮',
        '又胖就跟平面分不出來。',
  ...
  function areaA() {
    return TX.map((o) => {
      const cap = o.cap.map((s, i) => T(o.x + 16, 380 + i * 18, s, 'sub', null,
        o.wi.indexOf(i) >= 0 ? 'fill:var(--dg-warn)' : '')).join('');
      return `<g>${frame(o.x, CA_Y, CELL_W, CA_H)}
        ${T(o.x + 16, CA_Y + 26, o.no + '、' + o.nm, 'hd')}
        ${T(o.x + CELL_W - 16, CA_Y + 26, o.fa, 'lbl', 'end', 'fill:var(--dg-accent-2d)')}
        ${txCell(o.x, o.k)}
        ${T(o.x + 16, 364, '（剖面兩端的源／汲為示意，不與閘極同切面）', 'cap')}
        ${cap}</g>`;
    }).join('');
  }
  ...
  function foundryProcess() {
    ...
    return `<svg class="dg dgm dgfd dg1" viewBox="0 0 ${W} 1702" width="100%" style="display:block">${D.STYLE}
      ${T(16, 26, '晶圓代工：一顆電晶體與一個製程迴圈', 'ttl')}
      ${T(16, 46, '左：三代電晶體改的都是同一件事 —— 閘極能管到通道的幾個面（1 → 3 → 4）。下：晶圓廠不是一次做完，是同一個迴圈繞好幾十次。', 'cap')}
      ...
      ${T(16, 1616, '示意圖，非實物比例｜各層厚度與尺寸均為誇張放大', 'cap')}
      ${T(16, 1634, '本圖講矽邏輯製程；化合物半導體（GaAs／SiC／GaN）代工見「第三代半導體」那張', 'cap')}
      ${T(16, 1652, '本圖止於「一片做完的晶圓」；切割、封裝、測試見「傳統封裝與測試」那張', 'cap')}
      ${T(16, 1670, '節點效能數字為媒體整理（2026），會過期；本圖不寫任何良率、成本、市占率與產能數字', 'cap')}
    </svg>`;
  }
