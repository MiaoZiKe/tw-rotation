...
(function () {
  ...
  /* ★ 2026-09-26 覆蓋普查（scripts/_dg_overlap.py）：三個槓桿原本用「每 15 個字硬切一刀」斷行（跟字寬無關），
     閱讀模式一行伸出格子 26px、互相壓進隔壁那一格。改成 D.para 照最壞字寬、在標點處斷行，三格等高＝最高那格。*/
  const LEV = [['倉：自建還是租', '租來的衛星倉約期短、面積小，難導入自動化，揀貨效率與人力成本都壓不下來；自建大型物流中心才有空間放自動化設備。'],
    ['單：湊不湊得齊', '一張訂單如果在同一個倉湊不齊，就要從別的倉分開出貨 —— 一張單變成好幾筆配送成本。品項要擺對地方。'],
    ['最後一哩：自己送還是外包', '越快到貨，一趟車能送的件數越少。速度是賣點，也是成本。']];
  const LEVIN = 200 - 28;
  const LEVH = Math.max(128, Math.max.apply(null, LEV.map(([, t]) => 46 + D.para(0, 0, t, LEVIN).h + 4)));
  const levers = (y0) => LEV.map(([t, sub], i) =>
    part('ec_lever' + i, frame(16 + i * 212, y0 + 234, 200, LEVH)
      + T(30 + i * 212, y0 + 258, t, 'lbl')
      + D.para(30 + i * 212, y0 + 280, sub, LEVIN).svg)).join('');
  ...
  function areaGmv() {
    ...
    return T(16, y0 - 12, 'GMV、營收、毛利 —— 三個不同的數字', 'hd')
      ...
      + levers(y0)
      + D.para(16, y0 + 234 + LEVH + 18, '來源：倉儲轉型與衛星倉的結構性問題引自數位時代對富邦媒物流轉型的報導；金額與資本支出數字本圖不引用。', 628, { cls: 'cap' }).svg;
  }

  /* ================================================================ 章節 ②：兩檔台股與沒回答的事 */
  function areaWho() {
    const y0 = 1060, IN = 300 - 28;
    // ★ 2026-09-26 覆蓋普查：兩格與下面那一框原本是照 12px 寫死的斷行，閱讀模式伸出格子、伸出畫布最多 156px → 照最壞字寬重斷
    const L1 = ['線上零售平台；近年把倉儲從大量租用衛星倉轉向自建大型物流中心，目的就是把履約那一段的效率拉起來（數位時代報導）。'];
    const L2 = [{ t: '網路通路品牌（groups.yaml 的 note）。' }, { t: '★ 各家的履約成本占比、轉換率、客單價都查不到可引用的公開數字，不寫。', style: `fill:${C.warn}` }];
    const CH = Math.max(140, 66 + Math.max(D.para(0, 0, L1, IN).h, D.para(0, 0, L2, IN).h));
    const co = (i, code, name, lines) => {
      const x = 16 + i * 316;
      return part('ec_co_' + code, frame(x, y0, 300, CH)
        + R(x, y0, 4, CH, C.order)
        + T(x + 14, y0 + 24, name, 'lbl')
        + T(x + 14, y0 + 42, code, 'sub', null, `fill:${C.order}`)
        + D.para(x + 14, y0 + 66, lines, IN).svg);
    };
    const NO = ['轉換率、客單價、毛利率、市占率、履約成本占營收的比例 —— 沒有可引用的公司自述來源，一個數字都不寫。',
  ...
  function ecommerce() {
    return `<svg class="dg dgm rs dgec" viewBox="0 0 ${CW} 1400" width="100%" style="display:block">${D.STYLE}
      ...
      ${card({ part: 'ec_gmv', no: 5, side: 'l', color: C.gmv, ax: FX0 + 6, ay: 318, title: 'GMV ≠ 營收', sub: ['自營是把貨賣掉、GMV 幾乎全部認成營收；平台只認抽成那一段。比較兩家之前要先確認是哪一種。'] })}
