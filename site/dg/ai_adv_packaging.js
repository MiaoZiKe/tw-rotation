...
(function () {
  ...
  function icPackage() {
    ...
    return `<svg class="dg dgm rs icp" viewBox="0 0 ${CW} 2133" width="100%" style="display:block">${STYLE}${VARS}
      ...
      <text class="hd" x="322" y="78">主剖面：接點由下往上一路變小</text>   <!-- ★ 2026-09-26 覆蓋普查：原本 y 88，TIM2 那支往上的小箭頭尖端（y 88）戳進「變」字 → 標題上提 10 -->
      ...
      ${segCard('adv_pkg', '先進封裝', 11, 'µbump／C4／底填', '接點逐級放大；底填有圓角、補強環防翹曲', 630 + DX, 282)}   <!-- ★ 2026-09-26 覆蓋普查：原本 644，跟 12 號（659,286）只差 15px，兩顆編號圓點疊在一起、12 的字被 11 蓋掉；往左挪 14（仍在 C4／底填那一層上） -->
      ...
      { seg: 'osat_test', t: '上蓋與成品測試', s: 'TIM1、上蓋、燒機、分選出貨' }], 196, { cols: 3 })}
        ...
        <!-- ★ 2026-09-26 覆蓋普查：這一段的長句是照舊畫布寫的，閱讀模式（字大一階）在 660 寬裡伸出畫布 10～48px、尾巴被切掉。
             字一個都沒刪，只在標點處斷行；多出來的行由下面的 translate 整段往下推，不動任何圖形的相對位置。 -->
        <text class="cap" x="16" y="1596">再往外看一層：整條鏈由左到右的五站</text>
        <text class="cap" x="16" y="1614">（上面那一排就是這裡第三格「CoWoS 堆疊」拆開來的內部）。</text>
      ...
      { seg: 'abf_pcb', t: '上板', s: '載板 → 主機板' }], 196, { cols: 3 })}
        ...
        <text class="cap" x="16" y="1890">點零件篩的是「環節」，不是整個族群。</text>
        <text class="cap" x="16" y="1908">AI 先進封裝族群五檔（3711／3374／6271／6451／6789）裡，</text>
        <text class="cap" x="16" y="1926">只有 3711 與 3374 在本圖用到的環節名單上。</text>
        <text class="cap" x="16" y="1944">模封 EMC 與 TIM2 兩個零件沒有掛環節：</text>
        <text class="cap" x="16" y="1962">前者在本站沒有對應的一格（日系材料廠為主，查不到具名台股）、</text>
        <text class="cap" x="16" y="1980">後者通往的散熱器屬 AI 伺服器鏈。</text>
        <text class="cap" x="16" y="1998">去耦電容掛的是「被動元件」，它不在半導體鏈的環節名單上，</text>
        <text class="cap" x="16" y="2016">所以按那顆「環節」鈕會答「這個環節的台股不在本鏈成分股裡」</text>
        <text class="cap" x="16" y="2034">—— 那是正確答案，不是錯誤。</text>
        <text class="cap" x="16" y="2052">本圖畫「有上蓋」的型式，模封只包晶粒側面、頂面露出來接 TIM1。</text>
        <text class="cap" x="16" y="2070">資料來源與信心度全部列在 docs/diagram_specs/ic_package.md。</text>
        </g>`)}
    </svg>`;
  }
