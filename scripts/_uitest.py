...
SECTIONS = {
    ...
    # 兩張 CoWoS 去重、電路圖樣貌、動畫拉滿、圖高壓到 700px（DECISIONS #234）
    # ⚠ 名字裡的「批次21」是這件工作自己的批號，跟上面那行「輪動時鐘即時」的批號撞到了
    #   —— 兩者是同一天不同的兩件事，段落名不一樣所以 --only 不會誤選。
...
# ===================================================================== 批次13：配色與收納
DG_ROUTES_13 = [
    # ★ 2026-09-22：「半導體」那一列拿掉 —— 鏈層級的圖退場之後那個網址是圖別選單，
    #   量不到任何 SVG。它的內容已經在下面的「先進封裝」那一列裡。
    ("AI 伺服器", "ai_server"),
...
L1_BUDGET = {
    ...
    # ★ 2026-09-22：這個場景搬到族群層級的先進封裝那張圖上（DECISIONS #234），
    #   而且補了 C4／微凸塊兩層接點與正反面去耦電容，所以預算跟著重量一次。
    #   實測 draw call 129 -> 164、三角形 34,492。多出來的 35 個 draw call 就是那四個新零件
    #   （C4、微凸塊、正面去耦電容 6 顆、背面 LSC 3 顆）—— 這是「更細緻」的帳，不是失控。
    #   往後只准往下，不准再往上。
    "industry/semiconductor/dg/ai_adv_packaging": (164, 46000, 20000),
...
def t_clickbg(pg, base):
    ...
    pg.set_viewport_size({"width": 1440, "height": 900})
    pg.goto(DGH, wait_until="networkidle")
    # ★ 一定要 reload，不能只 goto（2026-09-22 實測到的假紅）。
    #   「零件誰做的」那一段用的是**同一個網址**，而且它會留下一個選著的零件；
    #   goto 到同一個 hash 不會觸發 hashchange、router 不會重畫，
    #   於是這裡量到的「基準」本身就已經有 sel 2／dim 17／小卡開著 ——
    #   點背景之後回到真正的基準（dim 8）反而被判成失敗。
    #   同一個 worker 裡段落是照 SECTIONS 的宣告順序跑的，所以這不是偶發，是必然。
    pg.reload(wait_until="networkidle")
    pg.wait_for_timeout(2200)
    _shut_side(pg)
    pg.evaluate("() => { const b = document.getElementById('dgAnim'); if (b) b.click(); }")
...
# ===================================================================== 批次21：兩張 CoWoS 去重 ＋ 電路圖樣貌 ＋ 動畫拉滿
#  DECISIONS #234（Andy 2026-09-22）：
#    ①「圖二 圖三 是否重疊題材，是的話幫我刪除一個」→ 鏈層級的 2D CoWoS 剖面退場，
#       兩塊獨門內容（每一層對應哪個環節、設計→…→上板 的整鏈流程）併進先進封裝那張
#    ②「不能看起來只有像是一般的方塊，他是電路圖就是要有電路圖的樣貌。有 IC 就有 IC 在上面」
#    ③「所有動畫都需要再更生動，動畫效果要拉滿，讓讀者更明白運作原理」
#
#  ⚠ 這一段每一條都驗**畫面真的因此改變了**，不是「元素存在」：
#     去重驗的是「舊網址真的變成選單、而且卡片點得進去」；
#     內容併入驗的是「環節標籤與流程列真的在新圖上，而且是真的環節名」；
#     小卡驗的是「真的列得出台積電與日月光」（改之前是 0 家，這條就是為了抓那件事）；
#     動畫驗的是「連續兩次取樣完全一樣」（CSS 與 SMIL 兩種都要）。
