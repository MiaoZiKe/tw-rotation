...
#   ② 環境貼圖、真陰影、透明比例、效能，各自有一條量出來的數字。
#
# ★ 色差用 **CIE76 ΔE**（Lab 空間的歐氏距離），不是 RGB 歐氏距離。
#   理由：RGB 距離對「深藍 vs 墨綠」這種都很暗的配對會嚴重高估（兩者 RGB 差 38、
#   人眼幾乎分不開），對「銀灰 vs 淡銅」這種都很亮的配對又會低估。
#   ΔE76 不完美（藍區偏差大），但它是**感知空間**的距離，拿來驗「分不分得開」比 RGB 誠實。
#   門檻 25：ΔE 2.3 是「看得出不一樣」的下限，25 大約是「一眼就是兩種顏色」。
DG3D_DE_MIN = 25.0
