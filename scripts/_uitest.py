...
SECTIONS = {
    ...
    # 批次14b：電感電阻石英（power_inductor）／連接器高速互連（ai_interconnect）／
    #          第三代半導體（wide_bandgap）三張剖析圖 ＋ 每個零件的「誰做的」對應
...
# ===================================================================== 批次14b：三張新剖析圖
#
# 這三段共用的判準只有一條：**驗「畫面真的因此改變了」**，不是「元素存在」也不是「有 render」。
# 每一段都自己開頁、自己收尾，跟別的段落沒有共用狀態（平行跑也不會互相汙染）。
#
#   · `power_inductor`（電感·電阻·石英）：規格書 docs/diagram_specs/passive_rlc.md §8
#   · `ai_interconnect`（連接器高速互連）：docs/diagram_specs/connector_hsio.md §8
#   · `wide_bandgap`（第三代半導體）：docs/diagram_specs/wide_bandgap.md §8
#
# ⚠ 第三張是**反向驗收**：它要證明的是「我們沒有偷偷掛一個錯的環節」——
#   supply_chain.yaml 的半導體鏈 14 格裡沒有一格對應第三代半導體，所以那張圖
#   一個 data-seg 都不能有。掛了就是在公開網站上宣稱一件錯的事。
...
# 多欄版面專用：**有沒有哪一行字跨過欄與欄之間的溝**。
# 為什麼要單獨量：字級與重疊都量得到「字太小」「字疊字」，但量不到
# 「④ 那一格的字伸進 ⑤ 那一格裡」—— 那一行字沒有跟任何字重疊、也沒有超出畫布，
# 兩個既有的檢查都是綠的，實際上版面已經壞了（2026-09-22 真的踩到一次）。
# 判準：**起點落在某一欄之內**的字，不准橫跨那一欄右邊的溝。
# 跨欄的整頁級文字（標題、說明、最底下那幾行）起點一律在最左邊（x 小於第一欄的左緣），
# 所以不會被誤判。
B14B_GUTTER = """(gaps) => {
  const svg = document.querySelector('#prodDiagram svg');
  if (!svg) return {present: false};
