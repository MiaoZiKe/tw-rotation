...
- **push 前跑三件事**：`pytest tests/ -q`、`python scripts/_preview.py`（真圖表庫走過所有頁面、抓文字重疊、手機寬）、
  ...
  - **★ 但有三種情況一定要跑完整一輪**（判斷用機器，不准靠記憶 —— 用
    `git diff -U0 <上次 push>..HEAD -- site/` 去 grep）：
    1. 改到 **`site/index.html` 的 `<style>`**（CSS 是全站共用，改一條可能弄壞任何一頁）；
    2. 改到 **`app.js` 的共用工具**：`function chart(`、`rangeBar(`、`playBar(`、`groupChips(`、
       `openZoom(`、`applyTheme(`、`route(`、`empty(`、`heatPanel(` 這幾支
       —— 它們被十幾個段落共用，只驗「這批的功能」必定漏掉別頁的回歸；
    3. Andy 說「這一批做完了，幫我全部驗一次」。
    ⚠ 這條規則的代價講在前面：**只驗對應段落，就是把「改 A 弄壞 B」的風險換成速度**。
    2026-09-20 的平行化實測就抓到三個「只在別的段落才會紅」的缺陷。
    所以每一批 push 完要在 HANDOFF 記一句「這批只驗了哪幾段」，
    下次有人回報怪象時才找得到「那批沒驗到哪裡」。
