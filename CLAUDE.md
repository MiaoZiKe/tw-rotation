# CLAUDE.md — 入口檔（新 session 第一個讀的）

## 這是什麼

**台股資金輪動儀表板**：每天盤後在 GitHub Actions 自動抓免費合規資料 → 算指標 → 產出靜態網站，
回答四個問題：**錢往哪個族群跑、那個族群現在貴不貴、什麼時候進場、有沒有理由不進場。**
判斷順序是刻意的：M1 資金面 + M2 基本面決定方向 → M3 技術面決定時機 → M4 事件面否決。

- 線上網址：<https://miaozike.github.io/tw-rotation/>
- Repo：<https://github.com/MiaoZiKe/tw-rotation>（**public**，所以任何金鑰、持股、成本價都不能進 repo）
- 擁有者 Andy。`groups.yaml` / `themes.yaml` / `supply_chain.yaml` 的成分他會自己校訂。
- **★ 判斷依據是證據，不是誰在問**（Andy 2026-09-19：「不要把我的身份加進來，
  不因為我是誰而決定，需要依目前找到的資訊執行」）。
  這條是因為我把「你是前台積電製程，這條你比我準」當成把問題推回去的理由 ——
  那不是判斷，那是迴避。正確的做法是：**把證據攤開、依證據決定；證據不夠就去查，查完再決定。**
  他的背景只用來**校準說明的深度**（半導體供應鏈不必從頭解釋），
  **不可以拿來當「所以這樣畫應該沒問題」或「所以這題給他決定」的理由**。
  同理，也不准用「讀者不懂」當放寬標準的藉口 —— 這是一個 public 的網站，任何人都看得到。
- Andy 的工作方式：**他只重新整理網頁、不按任何本機指令**。程式碼由 Claude 透過 GitHub API 推上 main，
  資料由 Actions 自己 commit。本機 `.bat/.ps1` 只是初次安裝用。

## 開始工作前，依序讀這三個檔

1. **`AGENTS.md`** — 九個專家 Agent 與 CEO 的分工、產出格式、審核清單（怎麼做事）
2. **`HANDOFF.md`** — 目前進度、已知 bug、**「下一步」**（從哪裡接手）
3. **`DECISIONS.md`** — 已拍板的決策（不要重新討論、不要「順手改回去」）

之後才看 `docs/v3_sources_spec.md`（資料源規格）與程式碼。

## 工作規則

- **★ 開工第一件事：先確認「推得上去」，再開始改東西。**
  `git push --dry-run origin deploy:main`。如果回 `access denied by the git proxy: ... not in this
  session's authorized repository set`，代表這個 session 沒有 repo 的寫入權限 ——
  **立刻告訴 Andy、先不要埋頭做**。原因：Andy 只看網頁，沒 push 等於他什麼都看不到；
  2026-09-13 就是這樣累積了 8 個 commit、他連續四次回報「縮放還在」，其實每一次都早就改好了。
  授權是**跟著 session 走的**，所以每開一個新 session 都要重驗一次。
- **開始前先 `git pull`**（雲端資料湖與工作流每天都在 commit；本機永遠可能落後）。
- **每次工作結束前更新 `HANDOFF.md`**（進度勾選、已知 bug、下一步、最後更新時間），**commit + push 到 main**。
  沒有 push 的工作等於沒做。
- **絕不 force push、絕不覆寫 `data/*.parquet`。** 雲端是資料的權威來源（`store.append()` 只增不改）。
- **★★★★ 2026-09-22：他已經下過的指令，不准再問一次 —— 有指令就執行。**
  Andy 的原話：「**為何沒有執行，我明明說要執行了**」「**這樣就浪費了時間跟 Token**」。

  那天實際發生的事：他先說「**執行下去，請一口氣把所有圖完成，若是有卡住的就先把其他張優先完成**」，
  後來又說「這部分（半導體分族群）**就先評估需要繪製幾張花多久時間**」。
  我交完評估之後，問他「要畫 1 張、6 張還是 15 張」——
  **但他早就回答過了：全部。** 那一問白白燒掉一輪來回。

  根因不是我不知道他講過什麼，是**我把「他要一份評估」誤讀成「他要在評估之後做一個決定」**。
  他要評估是想知道成本，不是想開一個決策點。

  規矩：
  1. **開口問他之前，先回頭查他自己講過的話。** 既有指令涵蓋得到 → **直接做**，不要問。
  2. **「給我評估／估一下時間」不是「停下來等我選」。** 交完評估就照既有指令繼續做，
     順手講一句「我照你之前說的全部做，先從 X 開始」就夠了。
  3. **唯一可以問的情況**：兩種做法會導出**實質不同的工作**，而且**既有指令沒涵蓋**。
     兩個條件要同時成立。只滿足一個就是自己挑一個做下去，把假設講明白。
  4. 問之前先自問一句：**「這題他是不是已經答過了？」** 答得出「他答過了」就不要問。
  5. ⚠ 這條跟「有疑問先上網查、不要把問題推回給 Andy」是同一件事的延伸：
     **那條說不准把查得到的事推給他，這條說不准把他已經答過的事再推給他一次。**

- **★★★★★★★ 2026-09-25：遇到問題先接著做，不要停下來等。**
  Andy 原話：「遇到問題先接著做，不要浪費時間，剩下的問題等全部做好在處理」。
  卡住的那一項記進 HANDOFF 的「待處理」，其他項照做、做完的照樣先部署；
  整批做完之後再回頭一次處理卡住的。回報時把卡住的列在最後一節。

- **★★★★★★★ 2026-09-24：Andy 說「回報進度」時，一律用這個格式回覆**（Andy 原話：
  「以後我說回報進度，你的回覆會是 Task有哪些，哪些已完成，哪些進行中，是否可加速，預計要多久，哪些還沒完成，預估完成時間」）：
  1. **Task 有哪些**：這一輪所有任務的清單（一行一件，用他聽得懂的名字，不用 commit 編號）。
  2. **已完成**：哪些已經上線（附台北時間），哪些做完但只交付檔案（原型、匯出包）。
  3. **進行中**：誰在做、做到哪一步（例如「關卡跑到第 8／14 段」）。
  4. **是否可加速**：每一件講「可以／不行」與理由（例如同一支檔案不能拆兩人做、關卡時間固定）；可以就講怎麼加速、代價是什麼。
  5. **預計要多久**：進行中的每一件，給「還要多久」與「預估上線時間（台北）」。
  6. **還沒開始／卡住的**：列出來，卡在誰（例如「等 Andy 提供條款欄位」）。
  7. **預估完成時間**：整批全部完成的台北時間。
  ⚠ 估時要誠實：講區間（例如「40～70 分鐘」）並說依據；估錯了下一次回報要講「上次估 X，實際 Y，原因」。
  表格優先，一眼看完；時間一律台北時間（`TZ=Asia/Taipei date`）。

- **★★★★★★ 2026-09-23（傍晚，最高優先，蓋掉「一次全部做完再找我」的部署時機）：
  做到一個段落就先部署，沒做完的繼續跑。**
  Andy 的原話：「**以後做到一個段落，就先部署，而其他未完成的繼續執行，
  幫我多新增一個 Agent 專門部屬的，這樣可以同部執行任務**」。

  那天實際發生的事：24 件功能全部做完，我為了等一輪驗收判定 38 條結構斷言，
  壓著不推超過一小時。Andy 問「為何不能先部署上去好的？不是很快嗎？」——
  **他是對的**。那 38 條對應的 12 張圖，收窄是**上一批就已經推上線的**，
  我壓著這一批根本保護不到他，只是讓他多等。而部署本身只要 45 秒到 6 分鐘。

  規矩：
  1. **一段功能做完、驗過、就推。** 不要為了湊整批而讓做好的東西躺著。
     「一段」＝ 使用者看得出差別的一件完整的事，不是「改到一半」。
  2. **部署交給 `deployer` Agent**（`.claude/agents/deployer.md`），
     它專職做「判斷能不能推 → 跑關卡 → 推 → 盯 Actions → 開線上確認版號 → 回報」。
     它跑的時候**其他 agent 繼續做沒做完的**，兩邊平行，不要互相等。
  3. **擋與不擋的判準**：
     · **會讓使用者直接踩到的 bug** → 擋，修完才推
     · **斷言對不上** → 先確認「那段程式碼是不是早就在線上了」。
       是的話壓著沒有意義，先推，把未收斂的寫在回報裡。
     · **半成品** → 永遠不推。
  4. ⚠ 這條**不影響**「push 前跑關卡」「push 後確認線上換掉了」
     「HANDOFF 記這批驗了哪幾段」—— 那些是安全網，一條都不准省。
  5. ⚠ **時間一律報台北時間**（容器跑 UTC，差 8 小時）。
     那天一路報 UTC 害 Andy 以為進度對不上。用 `TZ=Asia/Taipei date`。

- **★★★★★ 2026-09-23：不要先給我看、不要先問 —— 一次全部做完再找我。**
  Andy 的原話：「**等等不用給我看直接全部做完**」「**接下來 要將所有任務一次完成不要先問我 全部做完再找我**」。

  在這之前的做法是「做好一個方案先截圖給他挑，挑完再往下做」（例如剖析圖的四種風格、3D 的 v2／v3／v9）。
  那樣每一次都要停下來等他回一句，他嫌慢。**新的預設：**
  1. **有選項就全部做出來**，做成使用者自己可以切的（風格、配色、版面），**不要拿來當問他的理由**。
     四種風格就四種都上、兩種展開做法就兩種都上並挑一個當預設，把選擇權放進畫面而不是放進對話。
  2. **整批任務做完才回報一次**。中途不截圖、不問、不確認 —— 除非**做下去會弄壞既有功能**，
     那種才照舊先講一句（這條是 2026-09-21 那條「高風險改動先問」的唯一保留）。
  3. 回報時一次給：改了什麼、驗了哪幾段、網址、版號、以及**我自己判斷過但他可能不同意的地方**（讓他一次否決，不是一次一次問）。
  4. ⚠ 這條**不影響**「push 前跑關卡」「push 後確認線上換掉了」「HANDOFF 記這批驗了哪幾段」——
     那些是給我自己的安全網，不是給他的問題。

- **★★★ 2026-09-21（最新，蓋掉下面那條「先展示不要推」）：功能做完就先部署，讓 Andy 在線上確認。**
  Andy：「以後優先部署做好功能，我才能快速確認」。
  **預設 = 一件功能做完 → 跑它自己那幾段驗收（#223）→ 直接推 main → 部署 → 告訴他。**
  不要再累積成一大批等他喊「驗證」，也不要為了湊齊一批而讓做好的東西躺在 wip 上。
  理由很簡單：**他只看網頁**。東西沒上線，他就沒辦法確認，我們的來回就是盲的。
  - **一件一件推，不要綁在一起。** 三件功能分三次推，他可以一件一件確認；
    綁成一包推，他說「有問題」我還要先拆是哪一件。
  - `scripts/_show.py` 的截圖**不取代部署** —— 它現在的用途是
    「部署之前我自己先看一眼」與「解釋我改了什麼」，不是給他驗收用的。
  - `obsidian/unverified.yaml` 仍然留著，但**只登記「還沒部署」的東西**
    （例如卡在別人手上、或我判斷風險太高不敢直接上的）。清單應該常常是空的。
  - `claude/wip` 分支**只給「還沒做完」的在途工作**（多個 agent 同時改同一個檔那種）。
    做完就合併 main，不要停在 wip。
  - ⚠ 唯一的例外：**會弄壞既有功能的高風險改動**（改資料口徑、動到全站共用的東西）——
    那種先問他一句「這個要直接上還是先看截圖」，問完照他說的做。

- **（已被上面那條取代）2026-09-21 稍早的流程：改完先展示，不要馬上推。**
  Andy：「我想要把效率的問題處理，我覺得跑東西太久，每次修改都要全部重新測試太花時間…
  你改完之後可以先不用推上去，先在 Claude 這邊展示給我，並且還沒驗證的部分先記錄下來…
  之後我說驗證，再全部一起執行。初步功能我先人為測試，我確認後你再看細項」。

  以前每一次小改都要「跑關卡 → 推 → 等部署 → 他打開網頁看 → 不對 → 再來一輪」，
  一輪至少 15 分鐘，而**大部分來回其實只是在確認「長得對不對」**——
  那不需要關卡也不需要部署。新流程：

  1. **改完先截圖給他看**：`python scripts/_show.py --view flow --width 1440,390`
     （本機伺服器 ＋ 真圖表庫，秒級；`--sel` 只截某一塊、`--zoom 3` 看細節、
     `--click` 先點幾下再截、`--full` 整頁）。用 SendUserFile 把圖傳給他。
     **這支不是關卡** —— 它只證明「長這樣」，不證明「對」。
  2. **登記進 `obsidian/unverified.yaml`**：改了什麼、動到哪些檔、
     「驗證」時要跑哪幾段（照下面那張對照表挑）、他看過了沒、他說對了沒。
     跑 `python scripts/gen_unverified.py` 產出 `obsidian/120-未驗證清單.md`。
  3. **★ 先推到 `claude/wip` 分支，不要推 main。**
     這個容器是暫時的，**沒推的東西一旦容器被回收就消失**；
     而 `pages.yml` 只在 `main` 觸發，所以推 wip 不會部署、不會動到他看的網站。
     「先不推」的正確做法是「先推 wip」，不是「留在工作區」。
  4. **他說「驗證」** → 把清單裡所有 gates 的**聯集**一次跑完
     （`120-未驗證清單.md` 最後一節會直接把指令列出來）→ 合併到 main → 推 → 清空清單。
  5. 順序是**他先人工測基本功能，確認之後我才查細項（數據、口徑、邊界）**——
     不要反過來，先花時間查細項卻發現方向就不是他要的。

  ⚠ 例外：他明確說「這個先上」、或是修的是他**現在正在看**的東西，就照舊直接驗完推。

- **push 前跑三件事**：`pytest tests/ -q`、`python scripts/_preview.py`（真圖表庫走過所有頁面、抓文字重疊、手機寬）、
  `python scripts/_uitest.py`（**真人操作驗收**，含全站縮放掃描）。
  - **★ 只有純前端改動可以跳過 pytest**（Andy 2026-09-18 拍板，省 7.5 分鐘）。
    pytest 測的全是 Python 端（`pipeline/`、`compute/`、`indicators.py`），改 `site/*.js`
    在物理上不可能讓它變紅。**判斷一律用機器，不准靠記憶**：
    `git diff --name-only <上次 push 的 commit>..HEAD` —— 清單裡只要出現
    `pipeline/`、`tests/`、`scripts/`（`_uitest.py`、`_preview.py`、`_show.py`、
    `gen_unverified.py` 除外 —— 機器確認過 `tests/` 沒有 import 它們任何一支，
    只在註解裡被提到，所以它們壞掉在物理上不可能讓 pytest 變紅）、`requirements.txt`、`.github/`
    其中任何一項，**就一定要跑 pytest**；只有全部落在 `site/**` 才准跳過。
    反過來也一樣：`_preview.py` 與 `_uitest.py` 測的是前端，所以清單裡**完全沒有 `site/**`
    也沒有 `pipeline/build_payload.py`**（產出 JSON 的那支）時，那兩支可以跳過。
    只要沾到其中任何一個，就一定要跑。**跳過的時候要跟 Andy 講一聲跳了什麼、為什麼**，
    不可以默默省掉 —— 他沒辦法從對話裡看出我到底驗了什麼。
  - **★★ `_uitest.py` 預設只跑「這批需求對應到的功能」，不是整輪**
    （Andy 2026-09-21：「我覺得每次要這樣全部驗證太麻煩，以後改成我提出的內容對應功能做驗證」）。
    整輪 395 秒、對應段落通常 60～120 秒，一批省 5 分鐘。做法：
    `python scripts/_uitest.py --only <段落1>,<段落2>`，段落名照下面這張對照表挑，
    **並在回報裡寫清楚「這批我驗了哪幾段」** —— 他要看得出我驗了什麼、沒驗什麼。

    | 這批碰到什麼 | 要跑的段落 |
    |---|---|
    | 總覽頁 | `總覽`、`新-大盤三張圖`、`今日事件` |
    | 資金流向頁（排行／輪動時鐘／桑基／集中度） | `資金流向`、`新-資金流向`、`新-輪動時鐘`、`批次2`、`批次3`、`批次7` |
    | 產業地圖／產業鏈／關聯圖 | `產業`、`族群頁`、`產業鏈導覽`、`一般電子鏈`、`產業關係面板`、`新-產業與個股` |
    | 個股頁（K 線、多週期、營收、籌碼） | `個股`、`K線縮放`、`排序`、`新-產業與個股` |
    | 盤中即時／報價／分 K | `盤中即時`、`個股即時分K`、`市場明細` |
    | 題材熱度／3D 剖析圖 | `題材`、`批次6-N1`、`批次6-圖九`、`批次6-圖十` |
    | 季節性 | `季節性` |
    | 任務板 | `任務板` |
    | 版面／等高／欄寬／多寬度 | `新-版面等高與多寬度`、`縮放掃描`、`手機` |
    | 主題（深／淺）、設定面板 | `淺色主題`、`明亮主題`、`設定面板` |
    | 資料狀態、版號徽章 | `資料狀態`、`網頁版號` |

    ★ 2026-09-24 起也可以**從積木清單查**：`python scripts/_uitest.py --list-modules` 列出 27 塊積木各自對應的段落，
    `python scripts/_uitest.py --module <積木 id>`（例如 `--module broker.views`）只跑那塊積木宣告的段落。
    清單在 `site/modules.js`，新增或搬動積木時一起更新它。⚠ `--only` 是**子字串比對**（`批次2` 會帶到批次21～29），
    要精準請用 `--sections`。

  - **★★ 2026-09-21（第三次收斂）：沒動到的檔案不跑；動到的檔案，只跑「改動的那個地方」。**
    Andy：「驗收集測試部分 若是沒動到的檔案就不需要跑驗證，
    即使動到的檔案，也先跑原本改動的地方即可」。
    具體：
    1. **用 `git diff --name-only` 決定範圍，不准靠印象。** 檔案沒進清單 → 它對應的段落一律不跑。
    2. 一批工作裡每一件各自登記自己的 gates（`obsidian/unverified.yaml`）。
       他說「驗證」時**不要再取聯集重跑一輪** ——
       已經在自己那一輪跑綠、而且之後沒有人再動過它的檔案的，就不必再跑。
       只跑「上次跑綠之後檔案又被動過」的那些。
    3. 同一個檔案裡改了 A 功能，就跑 A 的段落；不要因為「這個檔案很大所以全跑」。
    ⚠ 代價：合在一起才會發作的互動（A 改動撞到 B 的狀態）就驗不到。
    配套仍然是**每批在 HANDOFF 記一句「這批只驗了哪幾段」** —— 那是唯一的安全網。
  - **★ 完整一輪只在一種情況下跑：Andy 說「幫我全部驗一次」。**
    （Andy 2026-09-21 第二次講：「驗證不要跑全部，針對需要的部分跑，太花時間了」。
     我原本把「改到 CSS」「改到 app.js 的共用工具」「全站共用資料換掉」列為例外，
     他看過之後**仍然要求只驗對應部分** —— 以他的為準，不要再自己加回例外。）
    ⚠ 代價寫在前面：**只驗對應段落，就是把「改 A 弄壞 B」的風險換成速度**。
    2026-09-20 的平行化實測就抓到三個「只在別的段落才會紅」的缺陷。
    所以配套是**每批 push 完要在 HANDOFF 記一句「這批只驗了哪幾段」**，
    下次有人回報怪象時才找得到「那批沒驗到哪裡」。這條配套不准省 ——
    它是這個取捨唯一的安全網。
    改到 CSS 或 `app.js` 的共用工具（`chart` / `rangeBar` / `playBar` / `groupChips` /
    `openZoom` / `applyTheme` / `route` / `empty` / `heatPanel`）時，
    **自己多挑幾段會用到它的段落加進去**，但不必跑滿 38 段。
  - **`_preview.py` 不在精簡範圍內，照跑**。它只要 2 分鐘，而且它抓的是
    文字重疊與多寬度溢出 —— 那正是「改 A 弄壞 B」最常見的形態，砍掉它省不到時間卻放掉最大的網。
- **★★ 2026-09-23 更正：這個容器打不開 <https://miaozike.github.io/tw-rotation/>。**
  實測 `curl` 回 `CONNECT tunnel failed, response 403`、HTTP 000；`WebFetch` 回 `EGRESS_BLOCKED`。
  所以**「我開過線上確認版號換掉了」這句話是假的，不准再講** ——
  2026-09-23 那天我對 Andy 講了很多次，全部是錯的。
  能做到的最強證據是：① Actions 每一步都成功 ② `deploy-pages` 有
  `Created deployment for <sha>` → `Reported success!` ③ Pages deployment 的 sha
  等於推的那個 commit、state 是 success。版號可以用同一個 API 自己算，但那是**推算值**。
  回報時一律講明「這是推算不是我親眼讀到的，請你重新整理看一眼」。
- **（已被上面那條更正）push 之後還沒完**：等 Actions 跑完，**真的打開 <https://miaozike.github.io/tw-rotation/> 確認線上版本換掉了**
  （比對右上角的**版號徽章**，那是 2026-09-16 為了這件事加的，見 DECISIONS #148），再跟 Andy 說「好了」。
  只說「推上去了」不算交付。
- **★ 每一次回報都要附上網址**（Andy 2026-09-18：「請每次都提供網址給我」）。
  他沒有辦法從對話裡判斷我說的「好了」是不是真的，所以每次交付都要給他三樣東西：
  **① 網址 ② 該看哪裡（版號徽章長什麼樣）③ 我自己已經開過線上確認的結果**。
- **★ 叫 Andy 按 .bat 之後，一定要回去確認那支 .bat 真的成功。**
  2026-09-18 踩到：`push-0918.bat` 在 `git pull --rebase` 那一步就失敗了
  （工作目錄有未提交的 `push-log-0916.txt`），完全沒推上去，而我沒有回查 ——
  Andy 打開網站看不到版號才發現。回查方法：讀他 repo 根目錄的 `push-log-*.txt`，
  或用瀏覽器看 `https://api.github.com/repos/MiaoZiKe/tw-rotation/commits`。
  **推送腳本一律用 `git pull --rebase --autostash`**，不要假設工作目錄是乾淨的。
- **★ .bat 的 `echo` 裡不准出現沒跳脫的 `>` `<` `|` `&`。**
  2026-09-18 踩到：我在提示訊息寫了 `deploy 14m26s -> ~3min`，
  cmd 把 `->` 的 `>` 當成**輸出轉向**，於是產生一個叫 `~3min` 的檔案，
  再被 `git add -A` 一起 commit 進 **public repo**。
  要寫就跳脫成 `^>`，或乾脆改用「to」「變成」這種字。
  打包完一律自檢一次：`grep -nE 'echo[^|]*[<>|&]' push-*.bat`，
  有中就確認每一個都跳脫過了。
- **★ Andy 的硬性要求：每批做完一定要「當自己是使用者，實際操作每個功能」。**
  每個按鈕真的按、每個輸入真的填、每個切換真的切、每條線真的用滑鼠拖出來，
  而且每一項都要驗**「畫面真的因此改變了」**（筆數變了／排序變了／localStorage 真的寫進去了），
  不是驗「元素存在」或「有 render」。這條規則是因為只看 render 的驗收放過了一堆錯
  （例如：切到沒資料的週期再切回日線，整張 K 線圖空白到重新整理為止）。`scripts/_uitest.py` 就是這件事的載體，
  **新功能一定要同時在裡面加一段真的操作它的驗收**。
- **★ 抓下來會重複用到的東西，一律存進資料湖，不准每次重抓**（DECISIONS #155）。
  判準一句話：**這份資料明天還會用到嗎？會 → 進 `config.TABLES` 走 `store.append()`；
  不會 → 才准當場抓當場丟。** 唯一例外是盤中即時報價（每分鐘就過期）。
  `build_payload` 對已經在湖裡的東西只准**讀**，不准再抓一次；抓取一律增量。
  這條是因為分 K 沒進湖，害每次部署都重抓 400 檔、白白多花 13 分 43 秒。
- **★ 別讓 Andy 等在「重跑一輪」上**（2026-09-18 他問「為何跑這麼久」）。
  三道關卡一輪 18 分鐘（pytest 7.5 ＋ _preview 2 ＋ _uitest 10），部署再 8～15 分鐘。
  那天總共跑了**三輪**，其中兩輪是我自己造成的：第一輪是驗收條件寫錯（把「選起來的零件本來就該提亮」
  判成「還在發光」），第二輪是推上線之後才發現窄畫面的 bug。三條具體的規矩：
  1. **關卡是驗完成品，不是驗半成品。** 東西真的做完、自己先在瀏覽器裡把每個新功能點過一遍，
     才跑三道關卡。不要「改一段跑一輪」。
  2. **開發過程就要驗窄畫面（800px）**，不要只在 1440px 看。2026-09-18 的 E6 縮圖 bug
     就是只驗寬螢幕放過去的 —— 他把瀏覽器縮成半邊就看得到，我卻是推上線之後才發現（DECISIONS #171）。
     `_uitest.py` 現在對新的版面元件一律要加窄畫面那一段。
  3. **追根因最多 10 分鐘。** 超過就改走「不依賴機制」的做法（例如「量完再用 transform 縮」
     而不是繼續研究 CSS 為什麼算錯），把觀察到的事實寫進 DECISIONS 就好。
     那天為了查一個「SVG 不吃 CSS 寬度」的怪象花了 25 分鐘，換路只花 5 分鐘。
- **★ 改了 `groups/*.yaml` 之後，跑 pytest 不夠 —— 要先重算 payload，前端關卡才驗得到新資料。**
  （2026-09-21 踩到，而且是**自己驗過才發現驗錯**的那一種。）
  那天補了三家進 `hdi_pcb`，照規矩跑了 pytest（458 passed）就推 main。
  後來被提醒「這樣會讓孤立節點超過棘輪上限」，我回頭跑
  `_uitest --only 批次6-圖十` —— **0 個問題**，以為是虛驚。
  實際上本機的 `site/data/*.json` 是**改 YAML 之前**建的，那一段測的是舊資料，**是假綠**。
  重算 payload 之後再跑，立刻變成「6 個孤立 / 上限 3」—— **main 當時真的是紅的**。
  所以規矩是：
  1. `git diff --name-only` 出現 `pipeline/groups/*.yaml` → **先 `python -m pipeline.build_payload`**
     （`SKIP_INTRADAY=1` 可省掉 Yahoo 分 K，約 5 分鐘），**再**跑 `_preview.py` 與 `_uitest.py`。
  2. 沒重算就跑前端關卡 ＝ **驗的是上一版的資料**，綠了也不算數。
  3. 雲端的 `pages.yml` 每次都會重算，所以**線上一定會吃到新資料** ——
     也就是本機假綠時，線上是真的壞的。這一條就是為什麼假綠比紅燈危險。
  ⚠ 順帶一條：**YAML 改動會撞到的不只 pytest**。`_uitest.py` 裡有依資料內容設的棘輪
  （例如 `SC_ISO_MAX` 限制「沒有邊的孤立節點」不准變多），補公司又沒有具名客戶可以連邊時
  一定會頂到它。補完公司要一起看那個數字，並把「為什麼這幾家沒有邊」逐家寫進註解。
- **★ 有疑問先上網查，不要用猜的、也不要把問題推回給 Andy**（Andy 2026-09-21：
  「當你遇到疑問，先上網查相關資料，理論上各大新聞媒體 投顧公司等平台，
  會有大量的資訊可以考證，CEO 分配人員下去進行」）。
  這條和開頭那條「判斷依據是證據，不是誰在問」是同一件事的兩半：
  **那條說不准用身份當理由，這條說證據要自己去拿。**
  - 工具：**只有 `WebSearch` 能用**（2026-09-21 分開實測）。
    `WebFetch` 和 `curl` 一樣被出口代理擋掉，回 `EGRESS_BLOCKED` ——
    連 `miaozike.github.io`（自己的網站）和 `www.moneydj.com` 都打不開。
    ⚠ 這條我第一次寫的時候寫成「WebSearch / WebFetch 都可用」，**是錯的**：
    我只測了 WebSearch 就把兩支一起寫上去。**分開測，不要推論。**
    - 能做的：用 `WebSearch` 下關鍵字，它會回標題、網址與摘要。**摘要就是我能拿到的全部**，
      點不進原文。所以引用時只能引摘要講的東西，不可以假裝讀過整篇。
    - 不能做的：讀某一個特定網址的內文、抓財報 PDF、看法說會簡報。
      需要原文時只能換關鍵字讓 `WebSearch` 再撈一次，或**老實跟 Andy 說這點查不到**。
    - **「我這邊連不到外面」只對「指定網址」成立**，不可以拿來當「所以我不查」的理由 ——
      `WebSearch` 永遠是開著的，不查就是沒做事。
  - 可用的查證面：各大新聞媒體（工商時報、經濟日報、Yahoo 股市、鉅亨）、
    投顧與研究平台（MoneyDJ、股感 StockFeel、豹投資、CMoney）、公司法說會與年報、
    以及原廠的產品頁與技術文件。
  - **CEO 的做法是分派專家 Agent 去查，不是自己硬幹**：
    半導體鏈 → `semi-chain-analyst`；載板／PCB／CCL／材料 → `pcb-substrate-analyst`；
    AI 伺服器（散熱／電源／光通訊／交換器／組裝）→ `ai-server-analyst`。
    這三支的產出格式本來就要求**附來源與信心度**。
  - **查完要留痕跡**：結論寫進 `docs/`（例如 `docs/groups_tide110_evidence.md` 那種），
    或寫進 YAML 的 `note`、程式碼註解。只寫在對話裡，下一個 session 就沒了。
  - ⚠ 查到的東西**不等於對的**。媒體會抄、投顧會有立場、AI 摘要會湊。
    兩個獨立來源對不起來時，以**公司自己講的**（法說會、年報、公開資訊觀測站）為準；
    還是對不起來就把分歧本身寫下來，標成低信心，不要挑一個順眼的當定論。
  - ⚠⚠ **`WebSearch` 的摘要會自己「補」出原文沒有的對應關係。** 實際踩到兩次：
    1. **假的公司對應**（2026-09-21）：查第三代半導體時，摘要寫出
       「穩懋（Transphorm）、宏捷科（Power Integrations）」—— **把兩檔台股直接譯成兩家不相干的美國公司**。
       沒有任何一篇原文這樣講，是摘要自己湊的。
       **判準：看到「中文名（英文名）」這種括號對應，一律要獨立查證**，不可以直接採用。
    2. **一文寫兩家被混著回答**（2026-09-21）：查 5469 瀚宇博拿到的 AI 數字，
       有一大半其實是子公司 6191 精成科的產能（來源是一篇同時寫兩家的稿）。
       **判準：查到數字要回頭確認「這個數字到底是誰的」**，特別注意母子公司與合併口徑。
    共通的處理：**摘要給的是線索，不是事實**。要寫進 YAML 或畫到圖上之前，
    用不同關鍵字再撈一次，看第二個獨立來源講不講得出同一件事。
- **★ 一律用繁體中文 —— 包含「跟 Andy 講話」本身**（Andy 2026-09-19：「記得幫我翻譯成繁體中文」
  ＋「請下定義 以後都會用繁體中文回答」）。
  範圍是**全部**：對話回覆、進度回報、關卡結果、表格的欄位名與內容、
  問他問題、解釋為什麼某件事做不到、程式註解、commit 訊息、文件、
  YAML 的 note、前端畫面上的每一個字。
  以前只寫了「註解、commit 訊息、文件」，所以我在回報裡混用英文標題與英文欄位名 ——
  他要看的是結論，不該還要先翻譯一次。
  **唯一的例外是不該翻譯的東西**：程式碼、識別字、檔名、路徑、指令、
  公司或產品的原名（NVIDIA、Broadcom、CoWoS、FinMind、Playwright、pytest）、
  以及引用原始錯誤訊息。專有名詞第一次出現時用「中文（原文）」的形式，例如
  「共同封裝光學（CPO）」，之後可以直接用縮寫。
- 金鑰只放 GitHub Secrets（`FINMIND_TOKEN`、`FRED_API_KEY`）與 Claude 的暫存區；**永遠不寫進 repo 任何檔案**。
  push 前 `git grep -iE "github_pat_|ghp_|finmind.*token" -- . ':!*.md'` 掃一次（排除文件本身的說明字串）。

## 絕對不要做的事（細節與理由在 DECISIONS.md）

1. 不碰 `www.twse.com.tw/rwd/...` 官網端點（使用條款禁爬）。可用的來源：`openapi.twse.com.tw`（日收，T-1）、
   `mis.twse.com.tw/stock/api/getStockInfo.jsp`（即時報價，**2026-09-14 加入白名單**，見 DECISIONS #108）、FinMind。
2. 不覆寫 `data/` 的 Parquet；一律 `store.append()`。
3. 不用執行當下日期當交易日；用回應裡的 `Date`。
4. 不引入 TA-Lib；指標自己算（`pipeline/indicators.py`、前端 `site/chart.js` 的 `KInd`）。
5. 不把持股部位、成本價、個人損益、任何 token 放進 repo。
6. 不把 `cancel-in-progress` 改成 true（會出現「repo 已更新但網站是舊版」）。
7. 不要爬券商分點（主力家數差沒有免費合規來源，已改用集保大戶增減＋法人動向替代）。

## Repo 結構導覽

```
.github/workflows/
  daily.yml      每日盤後管線（UTC 07:30/10:30/13:30＝台北 15:30/18:30/21:30，週一~五）：
                 15:30 那輪 --phase price 只抓價量；openapi 還沒給今天就用 mis 補（暫定值）
  backfill.yml   歷史回補（每小時，避開 09:20/10:20 UTC）：跑 --plan default，補齊即跳過
  pages.yml      site/** 或 pipeline/** 有 push 就重算 JSON 並部署
pipeline/
  config.py            所有路徑、端點、參數、TABLES（新增資料表先在這裡註冊 key）
  run_daily.py         每日管線；每步可失敗，寫 data/_state/last_run.json
  run_backfill.py      歷史回補；PLAN_DEFAULT 多步驟、402 即停、進度在 data/_state/backfill_progress.json
  build_payload.py     從資料湖算出前端 JSON（site/data/，不進版控）；SKIP_INTRADAY=1 可跳過 Yahoo 分 K
  indicators.py        MA/MACD/RSI/KD/ATR/SMC + technical_score()
  sources/             twse(OpenAPI) mis(即時報價/當天補齊) finmind tdcc tpex yahoo news macro
                       —— 全部回 DataFrame，失敗回空
  compute/             flow(M1) fundamental(M2) technical(M3規則) mtf(多週期SMC) stockpage season themes rrg
  groups/              ★ groups.yaml themes.yaml supply_chain.yaml（唯一人工維護）+ loader.py
  util/                roc(民國/千元/空值) http(重試+FinMind額度) store(Parquet append-only)
site/
  index.html app.js industry.js chart.js diagrams.js   前端（深色科技風；hash 路由）
  live.js            盤中即時層（每分鐘／盤後每 30 分／手動更新鈕；只更新 [data-live] 標記過的格子）
  vendor/echarts*.js  vendor/lightweight-charts.js       內建於 repo（Andy 公司網路擋 CDN）
  data/                                                  工作流產出，gitignore
data/                  Parquet 資料湖（雲端 Actions 每天 commit；本機只讀）
scripts/_preview.py    本機預覽驗證（Playwright + 真圖表庫）
scripts/stamp_assets.py 部署前給自家 JS/CSS 加版本戳（不加就要按 Ctrl+F5 才看得到新版）
tests/                 pytest，260 個
workers/quote-proxy/   Cloudflare Worker：即時報價的 CORS 代理（部署說明在 workers/README.md）
docs/                  v3_sources_spec.md（資料源規格）、截圖
```

## 前端路由（驗收時逐一打開）

`#overview` 總覽 ｜ `#flow` 資金流向（RRG／桑基／河流）｜ `#industry` 產業地圖 → `#industry/<chain>` 單一產業鏈 →
`#stock/<code>` 個股（分 K、多週期 SMC、營收／獲利／除權息／籌碼／基本資料）｜ `#themes[/id]` 題材熱度 ｜ `#season` 季節性

## 台股慣例（跟國外套件預設不同）

紅漲綠跌；KD 9,3,3 初始 50；RSI Wilder（SMA 種子，與 TradingView 對得上）；MACD 慣稱 DIF/MACD/OSC；
民國日期、千元、全字串 → `util/roc.py`。前端 `KInd` 與 Python 端口徑必須一致，改一邊記得改另一邊。
