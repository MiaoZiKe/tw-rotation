# AGENTS.md — 九個專家 Agent 與 CEO

這份文件定義專案的虛擬團隊。在 Claude Code / Cowork 裡的實作方式：**主 session 就是 CEO**，
九個專家用 `Agent` 工具（subagent）開，把下面對應段落整段貼進 subagent 的 prompt 當角色設定。
...
`.claude/agents/` 底下有九個 Agent 定義檔，Claude Code 會自動載入。
（原本六個；策略回測、圖解教學、資料治理三個在 2026-09-14 補上，見 ROADMAP 的缺口表。）
